...
class SSHTunnel:
    def __init__(
        ...
        self.tunnel_id = (
            tunnel_id or f"{hostname}:{local_port}->{self.target_ip}:{remote_port}"
        )
    ...
    def _accept_connections(self):
        ...
        while self.is_running:
            try:
                ...
                target_host = (
                    "127.0.0.1" if self.target_ip == self.hostname else self.target_ip
                )
                ...
                if not channel:
                    logging.error(
                        f"[{self.name}] 无法创建SSH通道到 {self.target_ip}:{self.remote_port}"
                    )
                    client_socket.close()
                    continue

                logging.debug(
                    f"[{self.name}] 为连接 {addr} 创建转发线程到 {self.target_ip}:{self.remote_port}"
                )
    ...
    def _forward_data(self, client_socket, channel):
        ...
        logging.debug(
            f"[{self.name}] 开始为连接 {connection_id} 转发数据到 {self.target_ip}:{self.remote_port}"
        )
    ...
    def start(self):
        if self.target_ip != self.hostname:
            logging.info(
                f"[{self.name}] 开始启动SSH隧道: {self.hostname}:{self.local_port}->{self.target_ip}:{self.remote_port}"
            )
        else:
            logging.info(
        ...
        try:
            ...
            if self.target_ip != self.hostname:
                logging.info(
                    f"[{self.name}] SSH隧道启动完成 (通过 {self.hostname} 连接到 {self.target_ip})"
                )
            else:
...
class MainWindow:
    ...
    def create_active_tunnels_tab(self):
        ...
        ttk.Label(input_frame, text="目标IP:").grid(
            row=0, column=4, padx=(20, 5), pady=5, sticky=tk.W
        )
        ...
        # 第二行：本地端口、远程端口
        ttk.Label(input_frame, text="本地端口:").grid(
        ...
        ttk.Label(input_frame, text="远程端口:").grid(
    ...
    def add_tunnel(self):
        ...
        logging.info(
            f"用户尝试添加隧道: SSH主机={hostname}, 目标IP={target_ip or hostname}, 名称={name}"
        )
        ...
        tunnel_name = (
            name or f"{hostname}:{local_port}->{final_target_ip}:{remote_port}"
        )

        if target_ip:
            logging.info(
                f"开始创建隧道: {tunnel_name} (通过 {hostname} 连接到 {target_ip})"
            )
        else:
    ...
    def update_table(self):
        ...
        for tunnel in self.tunnels:
            display_target_ip = (
                tunnel.target_ip if tunnel.target_ip != tunnel.hostname else "-"
            )
    ...
    def force_stop_tunnel(self, tunnel):
        logging.info(
            f"强制停止隧道: '{tunnel.name}' ({tunnel.hostname}:{tunnel.local_port}->{tunnel.target_ip}:{tunnel.remote_port})"
        )
    ...
    def stop_tunnel(self, tunnel):
        logging.info(
            f"用户请求停止隧道: '{tunnel.name}' ({tunnel.hostname}:{tunnel.local_port}->{tunnel.target_ip}:{tunnel.remote_port})"
        )
    ...
    def start_tunnel_from_config(self, config):
        ...
        if target_ip != config["hostname"]:
            logging.info(
                f"开始创建隧道: {config['name']} ({config['hostname']}:{config['local_port']}->{target_ip}:{config['remote_port']})"
            )
        else:
            logging.info(
...
class ConfigEditDialog:
    def __init__(self, parent, config, available_tags):
        ...
        ttk.Label(info_frame, text="SSH主机:").grid(
    ...
    def save_clicked(self):
        try:
            ...
            self.result = {
                ...
                "remote_port": remote_port,
                "name": name
                or f"{hostname}:{local_port}->{final_target_ip}:{remote_port}",
                "tags": selected_tags,
...
if __name__ == "__main__":
    ...
    root = tk.Tk(className="SSHTunnel")
    root.title("SSHTunnel")
    logging.info("创建主窗口")
